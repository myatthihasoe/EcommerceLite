<template>
    <Modal :open="open" @closeModal="closeModal" can-close-backdrop>
        ...
        >
            <h1 class="text-xl font-bold mb-5">Edit Attribute</h1>
            <div class="mt-10">
            ...
            <div class="mt-5">
                <!-- <div class="flex justify-end mt-5">
                    <button
                        type="button"
                        @click="addValue"
                        class="bg-blue-500 hover:bg-blue-700 text-white px-1 py-1 w-[150px] rounded"
                    >
                        + Add Value
                    </button>
                </div> -->
                <Label label="Values" />
                <div v-for="(value, index) in form.values" :key="index" class="mb-2 flex items-center">
                    <InputField
                        ...
                        readOnly='true'
                    />
                    <!-- <button type="button" @click="removeValue(index)" class="ml-4">
...
export default {
        ...
        attribute: {
            ...
            handler(newVal) {
                if (newVal) {
                    ...
                    this.form.values = newVal.attribute_options.map(option => option.value) || [];
                }
            },
        },
    },
    methods: {
        ...
        submit() {
            ...
            if (this.form.values.filter(value => value.trim() !== "").length === 0) {
                this.form.errors.values = "You must provide at least one value";
                return;
            }

            this.form.put(route('admin.attributes.update', this.attribute.id), {
                onSuccess: () => {
                    this.form.reset();
